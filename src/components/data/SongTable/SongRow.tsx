...
export const SongRow = ({
  ...
  const ACTIVE_ROW_STYLE: CSSObject = {
    backgroundColor: useColorModeValue("bgAlpha.300", "bgAlpha.700"),
  };
  ...
  const isRadio = !!data.playlist?.type && data.playlist.type.includes("radio");
  ...
  const clickFnWithStop = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      clickFn(e);
    },
      ...
      >
        ...
        {!hideCol?.includes("title") && (
          <HStack flex="1.4 1 90px" margin="auto">
            ...
            <TitleGrid
              song={song}
              onTitleClick={clickFnWithStop}
              flipNames={rowProps?.flipNames}
            />
          </HStack>
        )}
        {!hideCol?.includes("og_artist") ? (
          <Box flex="1 1 60px" noOfLines={1} px={2} margin="auto" opacity={0.8}>
